...
pushScope: IDENTIFIER {pushNewScope($1);printFuncName($1);};
...
statment: 
	...
	|IF {printIfCond("if");} OLIST stmnt_expression {printCond();} CLIST statment_block statment
	{
...
stmnt_expression:
	...
	//Logical operators
	| stmnt_expression AND stmnt_expression {$$ = mknode("&&", $1, $3);logiaclOperator("AND");setRight("&&");} 
	| stmnt_expression OR stmnt_expression {$$ = mknode("||", $1, $3);logiaclOperator("OR");setRight("||");}
	| NOT stmnt_expression {$$ = mknode("!", $2, NULL);logiaclOperator("NOT");setRight("!");}
	//Comparison operators
	| stmnt_expression COMPARE stmnt_expression {$$ = mknode("==", $1, $3);operatorisHere("COMPARE");setRight("==");}
	| stmnt_expression NOTEQUAL stmnt_expression {$$ = mknode("!=", $1, $3);operatorisHere("NOTEQUAL");setRight("!=");}
	| stmnt_expression GREATER stmnt_expression {$$ = mknode(">", $1, $3);operatorisHere("GREATER");setRight(">");}
	| stmnt_expression LESS stmnt_expression {$$ = mknode("<", $1, $3);operatorisHere("LESS");setRight("<");}
	| stmnt_expression GREATEREQUAL stmnt_expression {$$ = mknode(">=", $1, $3);operatorisHere("GREATEREQUAL");setRight(">=");}
	| stmnt_expression LESSEQUAL stmnt_expression {$$ = mknode("<=", $1, $3);operatorisHere("LESSEQUAL");setRight("<=");}
	//Arithmetic operatos
	| stmnt_expression PLUS stmnt_expression {$$ = mknode("+", $1, $3);setRight("+");}
	| stmnt_expression MINUS stmnt_expression {$$ = mknode("-", $1, $3);setRight("-");}
	| stmnt_expression MULTIPLY stmnt_expression {$$ = mknode("*", $1, $3);setRight("*");}
	| stmnt_expression DIVIDE stmnt_expression {$$ = mknode("/", $1, $3);setRight("/");}
	| ABSOLUTE  {setRight("|");} stmnt_expression   {setRight("|");} ABSOLUTE
	{
		$$ = mknode("|", mknode($2, NULL, NULL), mknode("|", NULL, NULL));
	}
	| address_expression {$$ = $1;}
...
expression: 
	...
	//variables, Constants and NULL 
	| INTEGER {$$ = mknode($1, NULL, NULL);addFuncCallArgType($1, "INTEGER");setRight($1);}
	| REAL {$$ = mknode($1, NULL, NULL); addFuncCallArgType($1, "REAL");setRight($1);}
	| CHAR {$$ = mknode($1, NULL, NULL);addFuncCallArgType($1, "CHAR");setRight($1);}
	| STRING {$$ = mknode($1, NULL, NULL);addFuncCallArgType($1, "STRING");setRight($1);}
	| BOOLEANTRUE {$$ = mknode($1, NULL, NULL);addFuncCallArgType($1, "BOOLEANTRUE");setRight("1,");}
	| BOOLEANFALSE {$$ = mknode($1, NULL, NULL);addFuncCallArgType($1, "BOOLEANFALSE");setRight("0,");}
	| IDENTIFIER {$$ = mknode($1, NULL, NULL);addFuncCallArgType($1, NULL);setRight($1);}
	| NULLL {$$ = mknode("NULL", NULL, NULL);};

expr: 
	OLIST expr CLIST {$$ = mknode("(", $2, mknode(")", NULL, NULL));}
	//Logical operators
	| expr AND expr {$$ = mknode("&&", $1, $3);setRight("&&");} 
	| expr OR expr {$$ = mknode("||", $1, $3);setRight("||");}
	| NOT expr {$$ = mknode("!", $2, NULL);addOperator("!");;setRight("!");}
	//Comparison operators
	| expr COMPARE expr {$$ = mknode("==", $1, $3);setRight("==");}
	| expr NOTEQUAL expr {$$ = mknode("!=", $1, $3);setRight("!=");}
	| expr GREATER expr {$$ = mknode(">", $1, $3);setRight(">");}
	| expr LESS expr {$$ = mknode("<", $1, $3);setRight("<");}
	| expr GREATEREQUAL expr {$$ = mknode(">=", $1, $3);setRight(">=");}
	| expr LESSEQUAL expr {$$ = mknode("<=", $1, $3);setRight("<=");}
	//Arithmetic operatos
	...
	| ABSOLUTE {setRight("|");} expr {setRight("|");} ABSOLUTE  
	{	
		$$ = mknode("|", mknode($2, NULL, NULL), mknode("|", NULL, NULL));
	}
	| address_expression {$$ = $1;}
	...
	| BOOLEANFALSE {$$ = mknode($1, NULL, NULL);addRightVar("BOOLEAN", NULL);setRight("0");}
...
address: ADDRESS IDENTIFIER {setRight("&");setRight($2);$$ = mknode("&", mknode($2, NULL, NULL), NULL);addOperator("&");addRightVar("IDENTIFIER", $2);}
	| ADDRESS OLIST IDENTIFIER CLIST {setRight("&");$$ = mknode("&", mknode("(", mknode($3, NULL, NULL), NULL), mknode(")", NULL, NULL));}
	| ADDRESS IDENTIFIER OINDEX str_expression CINDEX {setRight("&");$$ = mknode("&", mknode($2, NULL, NULL), mknode("[", $4, mknode("]", NULL, NULL)));}
	| ADDRESS OLIST IDENTIFIER OINDEX str_expression CINDEX CLIST {setRight("&");$$ = mknode("&", mknode("(", mknode($3, NULL, NULL), mknode("[", $5, NULL)), mknode("]", NULL, mknode(")", NULL, NULL)));}
	| expr ADDRESS expr {addOperator("^");setRight("&");};
...
deference_statement: DEFERENCE IDENTIFIER {setRight("^");setRight($2); $$ = mknode("^", mknode($2, NULL, NULL), NULL);addOperator("^"); addRightVar("IDENTIFIER", $2);}
	| DEFERENCE OLIST expr CLIST {setRight("^");$$ = mknode("^", mknode("(", $3, NULL), mknode(")", NULL, NULL));addOperator("^");}
	| DEFERENCE IDENTIFIER OINDEX expr CINDEX {setRight("^");$$ = mknode($1, mknode($2, NULL, NULL), mknode("[", $4, mknode("]", NULL, NULL)));}
	| deference_types DEFERENCE deference_types {addOperator("^");};

function_call: IDENTIFIER call_expression {printParam();printCallFunc($1); $$ = mknode("FUNC_CALL", mknode($1, NULL, NULL), mknode("ARGS", $2, NULL));
checkFuncProcCall($1);};
